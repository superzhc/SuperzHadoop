...
public abstract class RedisContainer implements RedisCommandsContainer, Closeable {
    ...
    @Override
    public void hmset(String key, Map<String, String> map, Integer ttl) {
        try (Jedis jedis = getInstance()) {
            jedis.hmset(key, map);
            if (ttl != null) {
                jedis.expire(key, ttl);
            ...
            if (LOG.isErrorEnabled()) {
                LOG.error("Cannot send Redis message with command HMSET to key {} and hashField {} error message {}",
                        key, map, e.getMessage());
            }
            throw e;
        }
    }
