 ...
 *
 * @author superz
 * @create 2023/3/30 11:52
 */
public class CronScheduledThreadPoolExecutor extends ThreadPoolExecutor
    ...
    boolean canRunInCurrentRunState(boolean periodic) {
        // superz note:通过反射来调用方法
        try {
            Method method = ThreadPoolExecutor.class.getDeclaredMethod("isRunningOrShutdown", boolean.class);
            method.setAccessible(true);
            return (boolean) method.invoke(this, periodic ? continueExistingPeriodicTasksAfterShutdown : executeExistingDelayedTasksAfterShutdown);
        } catch (Exception e) {
    ...
    private void delayedExecute(RunnableScheduledFuture<?> task) {
        ...
        try {
            if (isShutdown()) {
                Method method = ThreadPoolExecutor.class.getDeclaredMethod("reject", Runnable.class);
                method.setAccessible(true);
                ...
                if (isShutdown() &&
                    ...
                    Method method = ThreadPoolExecutor.class.getDeclaredMethod("ensurePrestart");
                    method.setAccessible(true);
                    method.invoke(this);
                    // ensurePrestart();
                }
    ...
    void onShutdown() {
        ...
        // superz note:通过反射来调用方法
        try {
            Method method = ThreadPoolExecutor.class.getDeclaredMethod("tryTerminate");
            method.setAccessible(true);
            method.invoke(this);
            //tryTerminate();
        } catch (Exception e) {
