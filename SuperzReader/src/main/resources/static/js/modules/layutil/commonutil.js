...
layui.define([], function (exports) {
    ...
    util.StringUtil = {
        ...
        isNest: function (rule, str) {
            ...
            var stack = [];
            for (var i = 0; i < len; i++) {
                if (util.ArrayUtil.inArray(keys, chs[i])) {
                    stack.push(rule[chs[i]]);
                } else {
                    if (chs[i] === stack[stack.length - 1]) {
                        stack.pop();
                    } else if (util.ArrayUtil.inArray(values, chs[i])) {
                        return false;
                    }
                }
            }
    ...
    util.DateUtil = {
        ...
        formt: function (dateTime, pattern) {
            ...
            if (util.StringUtil.isBlank(pattern)) {
                return date.toLocaleString();
            }
            return pattern.replace(/([a-z])\1*/ig, function (matchStr, group1) {
                var replacement = "";
                switch (group1) {
                    case util.DateUtil.patterns.PATTERN_ERA: //G
                        break;
                    case util.DateUtil.patterns.PATTERN_WEEK_YEAR: //Y
                    case util.DateUtil.patterns.PATTERN_YEAR: //y
                        replacement = date.getFullYear();
                        break;
                    case util.DateUtil.patterns.PATTERN_MONTH: //M
                        var month = date.getMonth() + 1;
                        replacement = (month < 10 && matchStr.length >= 2) ? "0" + month : month;
                        break;
                    case util.DateUtil.patterns.PATTERN_DAY_OF_MONTH: //d
                        var days = date.getDate();
                        replacement = (days < 10 && matchStr.length >= 2) ? "0" + days : days;
                        break;
                    case util.DateUtil.patterns.PATTERN_HOUR_OF_DAY1: //k(1~24)
                        var hours24 = date.getHours();
                        replacement = hours24;
                        break;
                    case util.DateUtil.patterns.PATTERN_HOUR_OF_DAY0: //H(0~23)
                        var hours24 = date.getHours();
                        replacement = (hours24 < 10 && matchStr.length >= 2) ? "0" + hours24 : hours24;
                        break;
                    case util.DateUtil.patterns.PATTERN_MINUTE: //m
                        var minutes = date.getMinutes();
                        replacement = (minutes < 10 && matchStr.length >= 2) ? "0" + minutes : minutes;
                        break;
                    case util.DateUtil.patterns.PATTERN_SECOND: //s
                        var seconds = date.getSeconds();
                        replacement = (seconds < 10 && matchStr.length >= 2) ? "0" + seconds : seconds;
                        break;
                    case util.DateUtil.patterns.PATTERN_MILLISECOND: //S
                        var milliSeconds = date.getMilliseconds();
                        replacement = milliSeconds;
                        break;
                    case util.DateUtil.patterns.PATTERN_DAY_OF_WEEK: //E
                        var day = date.getDay();
                        replacement = util.DateUtil.week['ch'][day];
                        break;
                    case util.DateUtil.patterns.PATTERN_DAY_OF_YEAR: //D
                        replacement = util.DateUtil.dayOfTheYear(date);
                        break;
                    case util.DateUtil.patterns.PATTERN_DAY_OF_WEEK_IN_MONTH: //F
                        var days = date.getDate();
                        replacement = Math.floor(days / 7);
                        break;
                    case util.DateUtil.patterns.PATTERN_WEEK_OF_YEAR: //w
                        var days = util.DateUtil.dayOfTheYear(date);
                        replacement = Math.ceil(days / 7);
                        break;
                    case util.DateUtil.patterns.PATTERN_WEEK_OF_MONTH: //W
                        var days = date.getDate();
                        replacement = Math.ceil(days / 7);
                        break;
                    case util.DateUtil.patterns.PATTERN_AM_PM: //a
                        var hours24 = date.getHours();
                        replacement = hours24 < 12 ? "\u4e0a\u5348" : "\u4e0b\u5348";
                        break;
                    case util.DateUtil.patterns.PATTERN_HOUR1: //h(1~12)
                        var hours12 = date.getHours() % 12 || 12; //0转为12
                        replacement = (hours12 < 10 && matchStr.length >= 2) ? "0" + hours12 : hours12;
                        break;
                    case util.DateUtil.patterns.PATTERN_HOUR0: //K(0~11)
                        var hours12 = date.getHours() % 12;
                        replacement = hours12;
                        break;
                    case util.DateUtil.patterns.PATTERN_ZONE_NAME: //z
                        replacement = util.DateUtil.getZoneNameValue(date)['name'];
                        break;
                    case util.DateUtil.patterns.PATTERN_ZONE_VALUE: //Z
                        replacement = util.DateUtil.getZoneNameValue(date)['value'];
                        break;
                    case util.DateUtil.patterns.PATTERN_ISO_DAY_OF_WEEK: //u
                        break;
                    case util.DateUtil.patterns.PATTERN_ISO_ZONE: //X
                        break;
                    default:
                        break;
                }
    ...
    util.ElementaryArithmeticUtil = {
        ...
        infixToPostfixExpression: function (expression) {
            expression = util.StringUtil.deleteWhitespace(expression);
            ...
            for (var i = 0; i < size; i++) {
                if (util.StringUtil.isNumeric(elementArr[i])) {
                    //如果是数值
        ...
        calculateByPrefixExpression: function (expression) {
            ...
            for (var i = size - 1; i >= 0; i--) {
                if (util.StringUtil.isNumeric(elementArr[i])) {
                    ...
                    switch (elementArr[i]) {
                        case '+':
                            result = util.ElementaryArithmeticUtil.add(operand1, operand2);
                            break;
                        case '-':
                            result = util.ElementaryArithmeticUtil.subtract(operand1, operand2);
                            break;
                        case '×':
                        case '*':
                            result = util.ElementaryArithmeticUtil.multiply(operand1, operand2);
                            break;
                        case '\/':
                        case '÷':
                            result = util.ElementaryArithmeticUtil.divide(operand1, operand2);
                            break;
                        default:
                            result = '';
        ...
        calculateByPostfixExpression: function (expression) {
            ...
            for (var i = 0; i < size; i++) {
                if (util.StringUtil.isNumeric(elementArr[i])) {
                    ...
                    switch (elementArr[i]) {
                        case '+':
                            result = util.ElementaryArithmeticUtil.add(operand2, operand1);
                            break;
                        case '-':
                            result = util.ElementaryArithmeticUtil.subtract(operand2, operand1);
                            break;
                        case '×':
                        case '*':
                            result = util.ElementaryArithmeticUtil.multiply(operand2, operand1);
                            break;
                        case '\/':
                        case '÷':
                            result = util.ElementaryArithmeticUtil.divide(operand2, operand1);
                            break;
                        default:
                            result = '';
                            alert("The operator 【" + elementArr[i] + "】 is not legal");
    ...
    util.RegexUtil = {
        ...
        createRegexObjMustExclude: function (input, conditions) {
            ...
            var subPattern = "";
            for (var i = 0; i < size; i++) {
                targetStrArr[i] = util.StringUtil.escapeMetacharacterOfStr(targetStrArr[i]);
                subPattern += targetStrArr[i];
                if (i != size - 1) {
                    subPattern += "|";
                }
            }
            regex = util.StringUtil.format(regex, [subPattern]);
            switch (matcherFlag) {
                case '0':
                    regex += "\\d";
                    break;
            ...
            regex += util.StringUtil.isNotBlank(length) ? "{" + length + "}" : "+";
            regex += "$";
