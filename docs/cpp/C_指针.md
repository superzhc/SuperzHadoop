...
> 在 C 语⾔中，⼀个指针可以指向 void 类型，即该指针对象的类型为 `void*`。如果⼀个指针对象是指向 void 类型的指针，那么该指针可被隐式转换为指向任⼀对象类型的指针。⽽指向任⼀对象的指针也都能被隐式转为指向 void 类型的指针。
>
> 因而 `void*` 指针类型往往在 C 语⾔社区中也被戏称为“万⽤指针类型”（universal pointer）。