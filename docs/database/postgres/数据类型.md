...
> `true` 状态的有效值可以是 `TRUE`、`t`、`true`、`y`、`yes`、`on`、`1`；`false` 状态的有效值为 `FALSE`、`f`、`false`、`n`、`no`、`off`、`0`
...
| `timestamp[p] with time zone`        | 8字节    | 包括日期和时间，带时区，简写成 `timestamptz` |
...
- `interval`:该类型描述了一个时间段的长度，单位可以是小时、天、月、分钟或者其他粒度。该类型适用于对日期和时间进行数学运算的场景。例如，假设从现在开始 666 天之后世界就会灭亡，那么在现在的时刻上加上长度为 666 天的一个 interval 类型值，就可以知道世界灭亡的准确时刻。
...
- `tstzrange`:该类型可用于定义 `timestamp with timezon`e 的开区间和闭区间。
...
```sql
-- 日期相加
select date '2024-04-24' + interval '1 day'
-- 日期相减
select date '20124-04-24' - interval '1 hour'
```
...
| 函数表达式                   | 参数                                                           | 描述                                             |
| ---------------------------- | -------------------------------------------------------------- | ------------------------------------------------ |
| `EXTRACT(field FROM source)` | field 值可以为 year、month、day、hour、minute、second、week 等 | 从时间数据类型中出年、月、日、时、分、秒、周几等 |
...
1. 使用花括号方式：`'{val1 delim val2 delim ...}'`，将数组元素值用花括号包围并用 delim 分隔符分开，数组元素值可以用双括号引用，delim 分隔符通常为逗号；
2. 使用 array 关键字，例如：`array[1,2,3]`

**数组元素查询**

如果查询数组所有元素值，值需要查询数组字段名称即可；数组元素的引用通过方括号`[]`方式，数组下标写在方括号内，编号范围为 `1` 到 `n`，`n` 为数组长度。

**数组元素追加**

数组元素的追加使用 `array_append` 函数，用法为 `array_append(anyarray, anyelement)`；数组元素追加到数组也可以使用操作符 `||`，例如 `array[1,2,3] || 4`

**数组元素删除**

数组元素的删除使用 `array_remove` 函数，用法为 `array_remove(anyarray, anyelement)`

**数组函数**

| 函数表达式                                                | 参数                                           | 返回值           | 描述                                   |
| --------------------------------------------------------- | ---------------------------------------------- | ---------------- | -------------------------------------- |
| `array_ndims(anyarray)`                                   |                                                |                  | 获取数组维度                           |
| `array_length(anyarray)`                                  |                                                |                  | 获取数组长度                           |
| `array_position(anyarray, anyelement)`                    |                                                |                  | 返回数组中某个数组元素第一次出现的位置 |
| `array_replace(anyarray,anyelement,anyelement)`           |                                                | 函数返回值为数组 | 数组元素替换                           |
| `array_to_string(anyarray, delim [, null_default_value])` | null_default_value：值为NULL的元素设置的默认值 |                  | 将数组元素输出到字符串                 |
