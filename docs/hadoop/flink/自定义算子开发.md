...
2. 自定义算子必须实现 `operate` 方法名的类，支持三种参数类型：`StreamExecutionEnvironment`、`DataStream<Row>`、`DataStream<Row>[]`
...
- 无上游节点，调用 `operate(StreamExecutionEnvironment)`
- 单输入上游节点，调用 `operate(DataStream<Row>)`
- 多输入上游节点，调用 `operate(DataStream<Row>[])`
