 ...
 *
 * @author superz
 * @create 2021/8/4 15:06
 */
public class MySQLSchema extends Schema implements Closeable {
    ...
    public MySQLSchema(String url, String username, String password, String database) {
        this(url, username, password, database, null);
    }

    public MySQLSchema(String url, String username, String password, String database, String tablePattern) {
        this.url = url;
    ...
    protected List<String> tables() {
        ...
        if (null != tablePattern && tablePattern.trim().length() > 0) {
            ...
            params = new Object[]{database, tablePattern};
        } else {
            params = new Object[]{database};
        }

        try {
            QueryRunner query = new QueryRunner();
            List<String> tableNames = query.query(getConn(), sql, params, new ColumnListHandler<String>("table_name"));
            return tableNames;
        } catch (Exception ex) {
    ...
    public static void main(String[] args) {
        try (MySQLSchema schema = new MySQLSchema("jdbc:mysql://pig-mysql:3307/bssp2?characterEncoding=utf8&zeroDateTimeBehavior=convertToNull&useSSL=false&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=GMT%2B8&nullCatalogMeansCurrent=true&allowPublicKeyRetrieval=true", "root", "root", "bssp2")) {
            System.out.println(schema.markdown("bssp2"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
