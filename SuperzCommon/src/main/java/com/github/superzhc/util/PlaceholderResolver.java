...
public class PlaceholderResolver
{
    ...
    public String resolveByRule(String content, Function<String, String> rule) {
        ...
        while (start != -1) {
            ...
            else if (placeholder.startsWith("date")) {// 2020年4月28日 superz 添加时间解析
                String pattern = "yyyy-MM-dd HH:mm:ss";// 默认格式
                if (placeholder.contains("#")) {
                    pattern = placeholder.substring(5);
                }
                replaceContent = LocalDateTime.now().format(DateTimeFormatter.ofPattern(pattern));
            }
            else {
